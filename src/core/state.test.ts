...
describe('parseState', () => {
	test('reads a file it wrote itself', () => {
		const text = serializeState({
			...
			selfUpdate: { lastCheckedAt: 1785844662000, latestVersion: '0.2.0' },
		});
	...
	test('a state written by a different version of the CLI is thrown away', () => {
		expect(parseState('{"version": 99, "selfUpdate": {"latestVersion": "9.9.9"}}')).toEqual({
			version: stateVersion,
		});
	});

	test('a field of the wrong type is dropped, never coerced', () => {
		const text = `{"version": ${stateVersion}, "selfUpdate": {"lastCheckedAt": "yesterday", "latestVersion": 2}}`;
		expect(parseState(text)).toEqual({ version: stateVersion, selfUpdate: {} });
	});

	test('an infinite timestamp does not survive a round trip through JSON either', () => {
		const text = `{"version": ${stateVersion}, "selfUpdate": {"lastCheckedAt": null}}`;
		expect(parseState(text).selfUpdate).toEqual({});
	});
});
...
describe('readState and writeState', () => {
	...
	test('what goes in comes back out', async () => {
		...
		await writeState(
			{ version: stateVersion, selfUpdate: { lastCheckedAt: 5, latestVersion: '1.0.0' } },
			path,
		);
		expect(await readState(path)).toEqual({
			version: stateVersion,
			selfUpdate: { lastCheckedAt: 5, latestVersion: '1.0.0' },
		});
	});

	test('it creates the folder rather than expecting one', async () => {
		const path = join(scratch, 'not', 'made', 'yet', 'state.json');
		await writeState({ version: stateVersion, selfUpdate: { latestVersion: '2.0.0' } }, path);
		expect((await readState(path)).selfUpdate?.latestVersion).toBe('2.0.0');
	});
